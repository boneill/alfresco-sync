...
define(["dojo/_base/declare",
   ...
   return declare([AlfSitesMenu, AlfXhr], {
       ...
       */
      _addMenuItem: function amc_AmcSitesMenu__addMenuItem(group, widget, index) {
    	 var sitePagelist;
    	 var siteCascade;  
    	if (group == this.favouritesList)
    	  {
    		  sitePageList = new AlfMenuGroup({
                  widgets: [{
    		  ...
    		  siteCascade = new AlfCascadingMenu(widget.config);
    		  //siteCascade.closeOnClick = false;
    	...
    	if (group == this.recentGroup)
  		  ...
  		  // Create the cascading menu item to popout the recent sites list...
  		  siteCascade = new AlfCascadingMenu(widget.config);
        ...
        //else
        //{
            // If we're not adding a Recent Sites or Favourites menu item then just default to the normal action
       ...
       */
      loadSitePages: function amc_AmcSitesMenu__loadSitePages(siteShortName, sitePageList) {
         ...
         //{
            ...
            this.serviceXhr({url : Alfresco.constants.URL_SERVICECONTEXT + "amc/site/" + siteShortName,
                             method: "GET",
       ...
       */
      sitePagesLoaded: function amc_AmcSitesMenu__loadSitePages(response, originalRequestConfig) {
         ...
         //popup.close(this.siteCascade.popup);
         ...
         {
            // TODO: Should add some error handling here 
         }
         // Re-open the popup (see AKU-354)...
         /**var self = group;
         siteCascade._openPopup({
            parent: group,
            ...
            onCancel: function(){ // called when the child menu is canceled
               if(focus){
                  ...
                  self.focusChild(this.siteCascade);
               }
