...
build_share() {
    ...
    yes | docker-compose -f "$COMPOSE_FILE_PATH" rm -f alfresco-sync-share
    $MVN_EXEC clean package -pl alfresco-sync-share,alfresco-sync-share-docker
}

build_acs() {
    docker-compose -f "$COMPOSE_FILE_PATH" kill alfresco-sync-acs
    yes | docker-compose -f "$COMPOSE_FILE_PATH" rm -f alfresco-sync-acs
    $MVN_EXEC clean package -pl alfresco-sync-integration-tests,alfresco-sync-platform,alfresco-sync-platform-docker
}
...
prepare_test() {
    $MVN_EXEC verify -DskipTests=true -pl alfresco-sync-platform,alfresco-sync-integration-tests,alfresco-sync-platform-docker
}

test() {
    $MVN_EXEC verify -pl alfresco-sync-platform,alfresco-sync-integration-tests
}
